...
export function applyOfflineLockDirective(
    ...
    // 黄金心墙体验法则：
    // 只有在【心墙数值真正改变】（增高加固 或 降低松动）、或是【初次关闭线下入口】时，
    // 才开启新一轮心防挑战，将 knockCount 归零；
    // 若角色在本轮回复中【心墙未变】（保持原样，normalizedRequired === prevRequiredKnocks），
    // 说明角色心境处于同一阶段，绝不能让用户白敲！必须坚决保留用户当前的 knockCount！
